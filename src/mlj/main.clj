...
(defn compile-run-file
  ...
  [filename]
  (let [module (do
                 (println "Creating Module " filename)
                 (time (create-module filename)))
        prog-str (do
                   (println "Reading in file " filename)
                   (time (slurp filename)))
        asts (do
               (println "Building AST")
               (time (parse-str prog-str)))
        env (do
              (println "Type checking")
              (time (type/check-prog asts)))
        prog (do
               (println "Compiling to Clojure")
               (time (compile-with-module asts nil)))]
    (do
      (println "Executing..")
      (time
       (binding [*ns* module]
